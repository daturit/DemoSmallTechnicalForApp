...
- (void)viewDidLoad
{
    ...
    // set up quene of AVPlayerItems
    NSArray *queue = @[[AVPlayerItem playerItemWithURL:[[NSBundle mainBundle] URLForResource:@"IronBacon" withExtension:@"mp3"]], [AVPlayerItem playerItemWithURL:[[NSBundle mainBundle] URLForResource:@"FeelinGood" withExtension:@"mp3"]], [AVPlayerItem playerItemWithURL:[[NSBundle mainBundle] URLForResource:@"WhatYouWant" withExtension:@"mp3"]]];
    ...
    void(^observerBlock)(CMTime time) = ^(CMTime time) {
        ...
        if([[UIApplication sharedApplication] applicationState] == UIApplicationStateActive) {
            ...
            NSLog(@"App is background. Time is: %@", timeString);
        }
    };
    
    self.timeObserver = [self.player addPeriodicTimeObserverForInterval:CMTimeMake(100, 1000) queue:dispatch_get_main_queue() usingBlock:observerBlock];
}
